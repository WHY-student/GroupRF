...
@HEADS.register_module()
class Mask2FormerRlnHead(MaskFormerRelationHead):
    def __init__(self,
                 in_channels,
                 feat_channels,
                 out_channels,
        ...
        # 添加随机token层训练
        self.token_num = 8
        self.learnable_token_feat = nn.Embedding(self.token_num, feat_channels)
        self.learnable_token_embedding = nn.Embedding(self.token_num, feat_channels)
    ...
    def forward(self, feats, img_metas):
        ...
        query_feat = torch.cat([query_feat, self.learnable_token_feat.weight.unsqueeze(1).repeat((1, batch_size, 1))],dim=0)
        query_embed = torch.cat([query_embed, self.learnable_token_embedding.weight.unsqueeze(1).repeat((1, batch_size, 1))], dim=0)
        ...
        for i in range(self.num_transformer_feat_level):
            ...
            attn_mask[torch.where(
                attn_mask.sum(-1) == attn_mask.shape[-1])] = False
            # cross_attn + self_attn
            attn_mask = F.pad(attn_mask, (0, 0, 0, self.token_num, 0, 0), value=False)
            ...
            cls_pred, mask_pred, attn_mask = self.forward_head(
                query_feat[:100,:,:], mask_features, multi_scale_memorys[
                    (i + 1) % self.num_transformer_feat_level].shape[-2:])
