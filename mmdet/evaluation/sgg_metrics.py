...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        pred_rels = np.array(local_container['pred_rels'])
        gt_rels = local_container['gt_rels']
        pred_classes = np.array(local_container['pred_classes'])
        gt_classes = np.array(local_container['gt_classes'])
        ...
        pred_triplets, pred_triplet_det_results, _ = \
            self.generate_triplet(pred_rels, pred_classes, pred_det_results)
